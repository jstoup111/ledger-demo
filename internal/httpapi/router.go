...
func handleAccountTransactions(store ledger.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Query().Get("format") == "csv" {
			body, err := renderCSV(transactions)
			if err != nil {
				log.Printf("render transactions CSV for account %q: %v", r.PathValue("id"), err)
				http.Error(w, "render transactions failed", http.StatusInternalServerError)
				return
			}
