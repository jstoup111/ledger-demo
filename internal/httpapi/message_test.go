...
func TestMessageForNamesSubmittedAmountForAmountRejections(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "malformed amount with a plausible decimal",
			identifier:   "amount_malformed",
			value:        "12.50",
		...
		{
			name:         "malformed amount with a plausible whole number",
			identifier:   "amount_malformed",
			value:        "500",
			want:         "Amount is malformed.",
			rejectsValue: true,
		...
		{
			name:         "malformed amount with a plausible negative decimal",
			identifier:   "amount_malformed",
			value:        "-12.50",
			want:         "Amount is malformed.",
			rejectsValue: true,
		...
		{
			name:         "malformed amount with a plausible zero decimal",
			identifier:   "amount_malformed",
			value:        "0.00",
			want:         "Amount is malformed.",
			rejectsValue: true,
		...
		{
			name:         "malformed amount with the largest plausible decimal",
			identifier:   "amount_malformed",
		...
		{
			name:       "malformed amount with script text",
			identifier: "amount_malformed",
			value:      "<script>alert(1)</script>",
			want:       "Amount is malformed. Submitted: <script>alert(1)</script>.",
		},
		{
